...
const ROLE_ENUM = ["user", "reviewer", "moderator", "admin"] as const;
const MEDIA_STATUS = ["pending", "verified", "deleted"] as const;
...
export const user = sqliteTable("user", {
  ...
  role: text("role", { enum: ROLE_ENUM }).notNull().default("user"),
...
export const roleChangeLog = sqliteTable("role_change_log", {
  ...
  previousRole: text("previous_role", { enum: ROLE_ENUM }).notNull(),
  newRole: text("new_role", { enum: ROLE_ENUM }).notNull(),
...
export const photo = sqliteTable("photo", {
  ...
  status: text("status", { enum: MEDIA_STATUS }).notNull().default("pending"),
}, (t) => [
